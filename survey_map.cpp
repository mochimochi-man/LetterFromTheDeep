...
void SurveyMap::draw(uint16_t* pixels,int width,int height,const Camera& camera,bool city,uint32_t monuments) const {
 if(!pixels || width<40 || height<40) return;
 int size=expanded?std::min(width-16,height-32):std::min(80,width/4);
 int left=expanded?(width-size)/2:width-size-5,top=expanded?16:5;
 ...
 for(int bz=0;bz<Blocks;++bz) for(int bx=0;bx<Blocks;++bx) {
  ...
  if(!seen) c=dim(c);
  for(int y=bz*size/Blocks;y<(bz+1)*size/Blocks;++y)
   for(int x=bx*size/Blocks;x<(bx+1)*size/Blocks;++x) pixel(left+x,top+size-1-y,c);
 }
 auto marker=[&](Vec3 p,uint16_t color,int radius) {
  int mx=int((p.x-WorldMin)/8),mz=int((p.z-WorldMin)/8);
  if(!visited(mx,mz,city)) return;
  int px=left+int((p.x-WorldMin)*size/WorldSize),py=top+size-1-int((p.z-WorldMin)*size/WorldSize);
  for(int y=-radius;y<=radius;++y) for(int x=-radius;x<=radius;++x)
   if(std::abs(x)+std::abs(y)<=radius && px+x>=left && py+y>=top && px+x<left+size && py+y<top+size) pixel(px+x,py+y,color);
 };
 ...
 int px=left+int(clampf((camera.position.x-WorldMin)*size/WorldSize,1,float(size-2)));
 int py=top+size-1-int(clampf((camera.position.z-WorldMin)*size/WorldSize,1,float(size-2)));
 ...
 auto fill=[&](const float* vx,const float* vy,uint16_t color) {
  ...
  for(int y=std::max(top,int(std::floor(loy)));y<=std::min(top+size-1,int(std::ceil(hiy)));++y)
   for(int x=std::max(left,int(std::floor(lox)));x<=std::min(left+size-1,int(std::ceil(hix)));++x) {
 ...
 auto arrowhead=[&](float grow,uint16_t color) {
  float vx[3]={px+dx*(len+grow),px-dx*(back+grow)-dz*(half+grow),px-dx*(back+grow)+dz*(half+grow)};
 ...
 // A yellow wedge alone does not say which end is the nose, so the nose is red.
 {
  ...
  float tipX=px+dx*len,tipY=py+dz*len;
  float leftX=px-dx*back-dz*half,leftY=py-dz*back+dx*half;
  float rightX=px-dx*back+dz*half,rightY=py-dz*back-dx*half;
