...
An undersea exploration for the ESP32-S3 and a 320x240 ST7789 panel. Everything is
drawn by a software rasteriser on the chip: no GPU, no prerendered frames.

## Genre

An ocean-exploration adventure game — or something that is not quite a game.
...
With nothing plugged into the USB port it runs a demo that tours every monument.
The ending, though, you can only reach by steering there yourself.
...
Pressing START during play opens the map and saves the current state. Each press
cycles map → monuments found → life found → normal view.
